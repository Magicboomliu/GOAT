...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        if 'cam_pos' in sample.keys():
            cam_pos = sample['cam_pos'] # [7,4]
            sample['cam_pos'] = torch.from_numpy(cam_pos)
        ...
        if self.img_height > ori_height or self.img_width > ori_width:
            ...
            if 'occu_left' in sample.keys():
                sample['occu_left'] = np.lib.pad(sample['occu_left'],
                                            ...
                                            constant_values=0)
            if 'gt_normal' in sample.keys():
                sample['gt_normal'] = np.lib.pad(sample['gt_normal'],
                                         ((top_pad, 0), (0, right_pad), (0, 0)),
                                         mode='constant',
                                         constant_values=0)
...
class RandomVerticalFlip(object):
    ...
    def __call__(self, sample):
        if np.random.random() < 0.5:
            ...
            if 'gt_normal' in sample.keys():
                sample['gt_normal'] = np.copy(np.flipud(sample['gt_normal']))
