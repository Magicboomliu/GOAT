...
class ModulatedDeformNormal(ModulatedDeformConv):

    def __init__(self, offset_channel, in_channels, out_channels,
                 kernel_size, stride, padding,
                 dilation=1, groups=1, deformable_groups=1, im2col_step=64, bias=True, lr_mult=0.1):
        '''
        Args:
            offset_channel: channels of normal based feature
            in_channles: channels of input features
            out_channels: 
            kernel_size: (3, 3)
            stride: 1
            
        '''
        super(ModulatedDeformNormal, self).__init__(in_channels, out_channels,
                                  kernel_size, stride, padding, dilation, groups, deformable_groups, im2col_step, bias)

        out_channels = self.deformable_groups * 3 * self.kernel_size[0] * self.kernel_size[1]
        self.conv_offset_mask = nn.Conv2d(offset_channel,
                                          out_channels,
                                          kernel_size=self.kernel_size,
                                          stride=self.stride,
                                          padding=self.padding,
    ...
    def forward(self, input_feature, normal_feature):
        out = self.conv_offset_mask(normal_feature)
        o1, o2, mask = torch.chunk(out, 3, dim=1)
        offset = torch.cat((o1, o2), dim=1)
        mask = torch.sigmoid(mask)
        return ModulatedDeformConvFunction.apply(input_feature, offset, mask, 
                                                self.weight, 
                                                self.bias, 
                                                self.stride, 
                                                self.padding, 
