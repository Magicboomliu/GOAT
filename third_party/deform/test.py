...
from modules.modulated_deform_conv import ModulatedDeformConv, _ModulatedDeformConv, ModulatedDeformConvPack, ModulatedDeformNormal
...
def example_normal():
    # get normal based features first
    input_feature = torch.randn(2, 64, 128, 128).cuda()
    input_normal = torch.randn(2, 3, 128, 128).cuda()
    normal_feature = torch.nn.Conv2d(3, 32, 3, 1, 1, bias=False).cuda()(input_normal)
    
    # feed normal features to deformable convolutions
    dcn = ModulatedDeformNormal(32, 64, 128, kernel_size=(3, 3), stride=1,
                padding=1, deformable_groups=2).cuda()

    output = dcn(input_feature, normal_feature)
    targert = output.new(*output.size())
    targert.data.uniform_(-0.01, 0.01)
    error = (targert - output).mean()
    error.backward()
