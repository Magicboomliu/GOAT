...
# Attention With the Positional Encoding
class AttentionRelative(nn.Module):
    ...
    def forward(self, fmap):
        ...
        if self.args=='position_and_content':
  
            sim_content = einsum('b h x y d, b h u v d -> b h x y u v', q, k)
            
            # sim_pos = self.pos_emb(q)
            sim_pos = 0
            sim = sim_content + sim_pos * 0.1
...
# Attention With the Positional Encoding
class AttentionRelative2(nn.Module):
    def __init__(
        self,
        *,
        dim,
...
if __name__ == "__main__":
    att = Attention(dim=128, heads=1,max_pos_size=160,args='none')
    fmap = torch.randn(2, 128, 40, 80)
    out = att(fmap)
