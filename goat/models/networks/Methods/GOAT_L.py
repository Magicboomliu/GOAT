...
from goat.models.networks.Backnbones.raft_extractor import BasicEncoderBig
from goat.models.networks.Attention.global_attention import AttentionRelative2
...
from goat.models.networks.Aggregation.transformer import FeatureTransformerV2
from goat.models.networks.Aggregation.transformer_utils import feature_add_position
from goat.models.networks.Update.OGAUpdate import OcclusionAwarenessGlobalAggregation, OcclusionAwarenessGlobalAggregationComplex14
...
class GOAT_L(nn.Module):
    def __init__(self,
        ...
        self.fnet = BasicEncoderBig(output_dim=256,norm_fn='instance',dropout=dropout)
        self.cnet = BasicEncoderBig(output_dim=256,norm_fn='batch',dropout=dropout)
        # feature aggregation
        self.feature_aggregation = FeatureTransformerV2(num_layers=4,d_model=256,
                                                      nhead=1,attention_type='swin',
                                                      ffn_dim_expansion=4,splits=4)
        
        # Context Attention Here
        self.att =AttentionRelative2(args='position_and_content', 
                             dim=cdim, 
                             heads=1, 
                             max_pos_size=240, dim_head=cdim)
        ...
        elif self.refine_type=='gl':
            self.update_block = OcclusionAwarenessGlobalAggregationComplex14(hidden_dim=128,
                                                                cost_volume_dimension=(2*self.sample_points-1) * self.num_levels,
                                                                prop_type='cspn')
    ...
    def forward(self,left_image,right_image,
        ...
        fmap1,fmap2 = feature_add_position(fmap1,fmap2,attn_splits=4,feature_channels=256)
        
        # feature aggregation
        fmap1,fmap2 = checkpoint(create_custom_self_attn(self.feature_aggregation),fmap1,fmap2)
        #fmap1,fmap2 = self.feature_aggregation(fmap1,fmap2)
        ...
        with autocast(enabled=True):
            ...
            # Context Feature
            inp = torch.relu(inp)
            attention = checkpoint(create_custom_self_attn(self.att),inp)
        ...
        disp3 = F.interpolate(disp_init,scale_factor=4.0,mode='bilinear',align_corners=False) *4.0
        ...
        occlusion_final = F.interpolate(occlusion_initial,scale_factor=4.0,mode='nearest')

        if test_mode:
            return cur_disp,disp_up,occlusion_initial

        return disparity_pyramid,occlusion_final

if __name__=='__main__':
    
    left_input = torch.randn(1,3,160,320).cuda()
    right_input = torch.randn(1,3,160,320).cuda()
    
    goat = GOAT_L(radius=3,num_levels=4,sample_points=4,dropout=0,refine_type='gl',up_scale=2).cuda()
    disparity_pyramid,occlusion_initial = goat(left_input,right_input)
    for d in disparity_pyramid:
        print(d.shape)
    
    print('------------------------')
    print(occlusion_initial.shape)
